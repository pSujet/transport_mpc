...
class Simulator3ADec:
    def __init__(self, model,controller1,controller2,controller3):        
        self.dt = model.dt
        self.n = model.n
        self.m = model.m
        self.model = model
    ...
    def run(self,init,simulation_time,DEBUG = False, verbose = True):
        ...
        x_warm1 = np.vstack((x_warm[0:13,:],x_warm[39:52,:]))
        x_warm2 = np.vstack((x_warm[13:26,:],x_warm[39:52,:]))
        x_warm3 = np.vstack((x_warm[26:39,:],x_warm[39:52,:]))
        u_warm1 = u_warm[0:6,:]
        u_warm2 = u_warm[6:12,:]
        u_warm3 = u_warm[12:18,:]
        ...
        xref1 = np.vstack([xref[0:13],xref[39:52]])
        xref2 = np.vstack([xref[13:26],xref[39:52]])
        xref3 = np.vstack([xref[26:39],xref[39:52]])
        xref_Log[:,[0]] = xref
        ...
        for i in range(self.n_sim):         
            ...
            x_Log1 = np.vstack([x_Log[0:13,i],x_Log[39:52,i]]).reshape((-1,1))  
            u_Log[0:6,[i]],x_temp1,u_temp1 = self.controller1.solve_mpc(x_Log1,xref1,x_warm1,u_warm1)
            t_stop = time.perf_counter()
            time_elapse1 = t_stop-t_start

            # -- solve mpc for agent 2
            t_start = time.perf_counter()
            x_Log2 = np.vstack([x_Log[13:26,i],x_Log[39:52,i]]).reshape((-1,1))  
            u_Log[6:12,[i]],x_temp2,u_temp2 = self.controller2.solve_mpc(x_Log2,xref2,x_warm2,u_warm2)
            t_stop = time.perf_counter()
            time_elapse2 = t_stop-t_start

            # -- solve mpc for agent 3
            t_start = time.perf_counter()
            x_Log3 = np.vstack([x_Log[26:39,i],x_Log[39:52,i]]).reshape((-1,1))  
            u_Log[12:18,[i]],x_temp3,u_temp3 = self.controller3.solve_mpc(x_Log3,xref3,x_warm3,u_warm3)
            t_stop = time.perf_counter()
            time_elapse3 = t_stop-t_start

            time_elapse = (time_elapse1+time_elapse2+time_elapse3)/3
            ...
            u_warm2 = u_temp1[:,0:]
            x_warm3 = np.hstack((x_temp3[:,1:],np.reshape(x_temp3[:,-1],(-1,1))))
            u_warm3 = u_temp3[:,0:]
            ...
            cost_Log[:,[i]] = (self.controller1.running_cost(x_Log1,xref1,self.controller1.Q,u_Log[0:6,[i]],self.controller1.R) \
                              + self.controller2.running_cost(x_Log2,xref2,self.controller2.Q,u_Log[6:12,[i]],self.controller2.R) \
                              + self.controller3.running_cost(x_Log3,xref3,self.controller3.Q,u_Log[12:18,[i]],self.controller3.R))/3      
    ...
    def plot_tension(self):
        ...
        axs[2].plot(t_x,direct_Log[2,:],color = 'darkorange')
    ...
    def animate(self,name = '',fps=10,bitrate=1000,dpi = 1000):
        x_Log = self.data['x'] 
        u_Log = self.data['u']
        ...
        def update_frame(i):
            ...
            ax.set_title("Decentralized Control",fontsize=20)
            ax.grid()    
            return ax
