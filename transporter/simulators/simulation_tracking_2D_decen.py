...
class Simulator2DDec:
    def __init__(self,model,controller1,controller2):        
        self.dt = model.dt
        self.n = model.n
        self.m = model.m
        self.model = model
    ...
    def run(self,init,simulation_time,DEBUG = False, verbose = True,SLEEP = True):
        ...
        x_warm1 = np.vstack((x_warm[0:6,:],x_warm[12:18,:]))
        x_warm2 = np.vstack((x_warm[6:12,:],x_warm[12:18,:]))
        u_warm1 = u_warm[0:4,:]
        u_warm2 = u_warm[4:8,:]
        if SLEEP:
            sleep_time = 5
        ...
        x_pred_Log = np.zeros((self.n_sim+1, self.n, self.controller1.Nt+1))
        ...
        err_v_Log = np.zeros((2, self.n_sim+1))
        err_the_Log = np.zeros((1, self.n_sim+1))
        err_thedot_Log = np.zeros((1, self.n_sim+1))

        # initialized values
        x_Log[:,[0]] = x0
        point = 0
        xref = xref_array[:,[point]]
        xref1 = np.vstack([xref[0:6],xref[12:18]])
        xref2 = np.vstack([xref[6:12],xref[12:18]])
        xref_Log[:,[0]] = xref
        err_x_Log[:,[0]] = xref[12:14] - x0[12:14]
        err_v_Log[:,[0]] = xref[14:16] - x0[14:16]
        err_the_Log[:,[0]] = xref[16] - x0[16]
        err_thedot_Log[:,[0]] = xref[17] - x0[17]
        ...
        for i in range(self.n_sim):  
            ...
            # -- solve mpc for agent 1
            t_start = time.perf_counter()
            x_Log1 = np.vstack([x_Log[0:6,i],x_Log[12:18,i]]).reshape((-1,1))          
            u_Log[0:4,[i]],x_temp1,u_temp1 = self.controller1.solve_mpc(x_Log1,xref1,x_warm1,u_warm1)           
            t_stop = time.perf_counter()
            time_elapse1 = t_stop-t_start

            # -- solve mpc for agent 2
            t_start = time.perf_counter()
            x_Log2 = np.vstack([x_Log[6:12,i],x_Log[12:18,i]]).reshape((-1,1))
            u_Log[4:8,[i]],x_temp2,u_temp2 = self.controller2.solve_mpc(x_Log2,xref2,x_warm2,u_warm2)
            t_stop = time.perf_counter()
            time_elapse2 = t_stop-t_start

            compute_time_Log[:,[i]] = (time_elapse1+time_elapse2)/2
            ...
            if verbose:
                print("Compute time MPC1: "+ str(time_elapse1))  
                print("Compute time MPC2: "+ str(time_elapse2))  
            ...
            x_warm1 = np.hstack((x_temp1[:,1:],np.reshape(x_temp1[:,-1],(-1,1))))
            u_warm1 = u_temp1[:,0:]
            x_warm2 = np.hstack((x_temp2[:,1:],np.reshape(x_temp2[:,-1],(-1,1))))
            u_warm2 = u_temp2[:,0:]
            ...
            cost_Log[:,[i]] = (self.controller1.running_cost(x_Log1,xref1,self.controller1.Q,u_Log[0:4,[i]],self.controller1.R)+self.controller2.running_cost(x_Log2,xref2,self.controller2.Q,u_Log[4:8,[i]],self.controller2.R))/2
            err_x_Log[:,[i+1]] = xref[12:14] - x_Log[12:14,[i+1]]
            err_v_Log[:,[i+1]] = xref[14:16] - x_Log[14:16,[i+1]]
            err_the_Log[:,[i+1]] = xref[16] - x_Log[16,[i+1]]
            err_thedot_Log[:,[i+1]] = xref[17] - x_Log[17,[i+1]]

            if verbose:
                print("MPC cost1: " + str(self.controller1.running_cost(x_Log1,xref1,self.controller1.Q,u_Log[0:4,[i]],self.controller1.R)))
                print("MPC cost2: " + str(self.controller1.running_cost(x_Log2,xref2,self.controller2.Q,u_Log[4:8,[i]],self.controller2.R)))
                print("Error position: " + str(np.linalg.norm(err_x_Log[:,[i+1]])))
            ...
            if np.linalg.norm(err_x_Log[:,[i+1]]) <= position_trigger_error and abs(err_the_Log[:,[i+1]])<=angle_trigger_error:
                ...
                # print('sleep time: '+str(sleep_time_count))
        ...
        data['err_v'] = err_v_Log
        data['err_the'] = err_the_Log
        data['err_thedot'] = err_thedot_Log
        data['cost'] = cost_Log
        ...
        data['ulim'] = self.controller1.ulim
    ...
    def animate(self,save = False,name = '',fps=10,bitrate=1000,dpi = 1000, PREDICT = False):
        ...
        def update_frame(i):
            ...
            if PREDICT:
                plot_system(ax,i,x_pred_Log[i,:,int(self.controller1.Nt/2):int(self.controller1.Nt/2)+1],self.model, alpha = 0.2,PLANAR=True)
                plot_system(ax,i,x_pred_Log[i,:,self.controller1.Nt:],self.model, alpha = 0.5,PLANAR=True)
