...
class NonlinearMPC2DDec:
    '''
    Nonlinear MPC Framework for decentralized control
    Conformity Symmetric Design of Decentralized Controllers for Multi-Agent Systems
    '''
    def __init__(self,model,N,Q,P,R,dt,ulim = 1.4,lambda_reg = 0,LINEARIZED = False, REDUCED = False,GUESS = False) -> None:
        self.model = model
        self.dt = dt
        ...
        self.create_solver(LINEARIZED,REDUCED,GUESS)
    
    def create_solver(self,LINEARIZED,REDUCED,GUESS):
        ...
        if GUESS:
            self.Tg = self.opti.variable(2,self.Nt)                   # Tension guess i vector form 2x1
        ...
        for i in range(self.Nt):
            if LINEARIZED:
                ...
                if REDUCED:                    
                    ...
                    if GUESS:        
                        self.opti.subject_to(self.X[:,i+1] == self.model.feulctrlTg(self.X[:,i], self.U[:,i],self.dt, self.lg_multiplier[:,i], self.Tg[:,i]))     
                        self.opti.subject_to(self.opti.bounded(-eps,self.model.psi1ddotTgF(self.X[:,i], self.U[:,i],self.lg_multiplier[0,i], self.Tg[:,i]), eps))
                    else:                            
                        # -- RK4
            ...
            # -- Tg case
            if GUESS:
                self.opti.subject_to(self.model.ctrlbF(self.X[:,i], self.U[:,i]).T @ self.Tg[:,i] <= 0)
                

        objective += -self.model.ctrldist1F(self.X[:,i], self.U[:,i])*self.lambda_reg
    ...
    def set_cost_functions(self):
        ...
        err_w1 = x[5] - xr[5]
        err_pvL = x[6:10] - xr[6:10]
        err_theL = quatdist(yaw2quaternion(x[10]),yaw2quaternion(xr[10]))
        err_wL = x[11] - xr[11]
