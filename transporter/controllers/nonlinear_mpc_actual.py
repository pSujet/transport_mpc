...
class NonlinearMPCOptiActual:
    '''
    Nonlinear MPC Framework with Opti for actual robot/gazebo
    '''
    ...
    def create_solver(self,LINEARIZED,REDUCED):
        ...
        self.Uded = self.opti.parameter(self.Nu,1)         # decrease input limit according to equivalent force
        if LINEARIZED:
    ...
    def set_cost_functions(self):
        ...
        # err_the1 = x[4] - xr[4]        
        # err_the1 = ca.if_else(err_the1 > np.pi,2*np.pi - err_the1,ca.if_else(err_the1 < -np.pi,2*np.pi + err_the1,err_the1))   # compute the closest angle [0,pi]
        ...
        # err_the2 = x[10] - xr[10]
        # err_the2 = ca.if_else(err_the2 > np.pi,2*np.pi - err_the2,ca.if_else(err_the2 < -np.pi,2*np.pi + err_the2,err_the2))   # compute the closest angle [0,pi]
        err_the2 = quatdist(yaw2quaternion(x[10]),yaw2quaternion(xr[10]))
        err_w2 = x[11] - xr[11]
        err_pvL = x[12:16] - xr[12:16]
        # err_theL = x[16] - xr[16]
        # err_theL = ca.if_else(err_theL > np.pi,2*np.pi - err_theL,ca.if_else(err_theL < -np.pi,2*np.pi + err_theL,err_theL))    # compute the closest angle [0,pi]
