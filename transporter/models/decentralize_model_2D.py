...
class DecentralizedModel2D:
    '''
    Decentralized model of the load transportation system
    ...
    def __init__(self,rdc,Rdc1,Rdc2) -> None:
    ...
    def control_dynamics(self):
        """
            DECENTRALIZED DYNAMICS
                (1)--(L)
        """
        x = ca.MX.sym('x',self.Nx,1)                                     # 6 DoF from one robot and load (6+6)
        u = ca.MX.sym('u',self.Nu,1)                                     # input of one robot
        ...
        # -- constant in equilibrium direction
        Tv2 = 0.5*-b
        ...
        self.psi1ddotTgF = ca.Function('psi1ddotF',[x,u,lg_multiplier,Tg],[psi1ddot], ['xk', 'uk','lg_multiplier','Tg'], ['psi1ddot'])


        dxdt = ca.vertcat(p1dot,v1dot,the1dot,w1dot,pLdot,vLdot,theLdot,wLdot)

        self.ctrl_dynamics = ca.Function('ctrl_dynamics', [x, u, lg_multiplier], [dxdt], ['x', 'u', 'lg_multiplier'], ['dxdt'])
        
        # -- Tg guess
        self.ctrl_dynamicsTg = ca.Function('ctrl_dynamics', [x, u, lg_multiplier,Tg], [dxdt], ['x', 'u', 'lg_multiplier','Tg'], ['dxdt'])
        
        # set function
        self.ctrln1F = ca.Function('n1F',[x,u],[n1], ['xk', 'uk'], ['n1'])
        self.ctrlbF = ca.Function('bF',[x,u],[b], ['xk', 'uk'], ['b'])
        self.ctrldist1F = ca.Function('dist1',[x,u],[dist1], ['xk', 'uk'], ['dist1'])
    ...
    def rk4_integrator(self):
        ...
        u = ca.MX.sym('u', self.Nu, 1)
        Tg = ca.MX.sym('Tg',2,1)
        xfeuler = x0 + dt * self.ctrl_dynamicsTg(x0, u, lg_multiplier, Tg)
        self.feulctrlTg = ca.Function('feulctrl', [x0, u, dt,lg_multiplier, Tg], [xfeuler], ['xk', 'uk','dt','lg_multiplier', 'Tg'], ['xfctrl'])
